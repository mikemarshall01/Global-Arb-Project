...
def gpw_calculation(crude, destination, arb_data, refinery_config):
    
    assay = arb_data.assay
    ports = arb_data.ports
    total = arb_data.total
    sub_to_ws = arb_data.sub_to_ws
    pricing_centre = ports[ports['PortName'].str.lower() == destination.lower()]['SubRegionName'].map(sub_to_ws['Price_Set']).to_string(index = False)    
    ...
    def standard_ref(crude, refinery_volume, reformer_capacity, fcc_capacity, coker_capacity, lvn_gasolinepool, kero_gasolinepool):
        ...
        def fcc():
            ...
            if  ports[ports['PortName'] == destination]['SubRegionName'].map(sub_to_ws['Price_Set']).iat[0] is 'HOUSTON':
    ...
    def calculate_margin(yields):
        ...
        products = [i for i in price_codes[pricing_centre].keys() if i != 'base_blend']
                
        """ Need to get a list of the codes from the relevant dictioanries"""
        price_codes_list = [price_codes[pricing_centre][i]['Code'] for i in yields.keys()]
        prices = total[price_codes_list]
        prices.columns = products
        
        """Get a list of the common products"""
        conversions =  [price_codes[pricing_centre][i]['Conversion'] for i in products]
        assign_dict = dict(zip(products, conversions))
        conversion_df = prices.assign(**assign_dict)
        converted_prices = prices.div(conversion_df)
        
        """Create df for yields and use to get product vaalues"""
        yld = converted_prices.assign(**yields)
        gpw = converted_prices.mul(yld)
        gpw['GPW'] = gpw.sum(1)
...
if __name__ == "__main__":
    ...
    test = gpw_calculation(crude, destination, arb_data, arb_data.refinery_configs['simple'])
